...
import { EMAIL_WELCOME_TEMPLATE } from "../config/emailTemplate.js";
...
export const userResistration = async (req, res) => {
  try {
    ...
    if(roles === "shopKeeper" || roles === "delivary") {
    ...
    const mailOptions = {
      ...
      html: EMAIL_WELCOME_TEMPLATE.replace("{{email}}", newUser.email).replace(
        "{{roles}}",
        newUser.roles.join(", ")
      ),
    };
